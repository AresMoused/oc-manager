...
export interface BuilderItem {
  ...
  tags: string;
  hex?: string;
  /** Preview image URL (CDN), preferably 128×128 webp */
  image?: string;
}
...
/** localStorage key for generator section on/off (shared with 抽卡姬) */
export const SECTION_ENABLED_KEY_PREFIX = "oc-gen-sec-on-";

export function loadSectionEnabledMap(
  sections: { key: string }[]
): Record<string, boolean> {
  const en: Record<string, boolean> = {};
  if (typeof window === "undefined") {
    for (const s of sections) en[s.key] = true;
    return en;
  }
  for (const s of sections) {
    const saved = localStorage.getItem(SECTION_ENABLED_KEY_PREFIX + s.key);
    en[s.key] = saved === null ? true : saved === "1";
  }
  return en;
}

export function setSectionEnabledKey(key: string, on: boolean) {
  if (typeof window === "undefined") return;
  localStorage.setItem(SECTION_ENABLED_KEY_PREFIX + key, on ? "1" : "0");
}

/** Filter builder to sections enabled in the appearance generator toggles. */
export function filterEnabledSections(data: BuilderData): BuilderData {
  const en = loadSectionEnabledMap(data.sections);
  return {
    ...data,
    sections: data.sections.filter((s) => en[s.key] !== false),
  };
}
...
export function composePrompt(
  ...
  opts?: { respectSectionEnabled?: boolean }
): string {
  const src =
    opts?.respectSectionEnabled === true ? filterEnabledSections(data) : data;
  let out = (src.base || "") + (src.fixed || "");
...
/**
 * Parse CSV text with columns: 名字/name , 提示词/prompt/tags
 * Header row is optional (detected by common header names).
 */
export function parseCsvNamePrompt(
  text: string,
  opts?: {
    sectionKey?: string;
    sectionLabel?: string;
  ...
  const headerHints = ["名字", "名称", "name", "title", "提示词", "prompt", "tags", "tag"];
  if (first.some((c) => headerHints.includes(c))) {
    start = 1;
  }
...
/** @deprecated kept for compatibility — prefer parseCsvNamePrompt */

      ...
      {textImportOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-[#111] border border-neutral-700 rounded-xl p-5 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-xs text-neutral-500 leading-relaxed">
              每行一条：<code className="text-neutral-300">名字: 提示词</code>
              。完整支持简体中文，不依赖 Excel 编码。适合手写或从文档粘贴。
            </p>
            <div className="rounded-lg border border-neutral-800 bg-[#0c0c0c] p-3">
              <div className="text-xs text-neutral-400 mb-1">参考范例</div>
              <pre className="text-[11px] font-mono text-neutral-400 whitespace-pre-wrap leading-relaxed">{"金发: blonde hair, long hair,\n红眼: red eyes, glowing eyes,\n黑色夹克: black jacket, leather jacket,"}</pre>
            </div>
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      if (!textCatalogName.trim()) {
                        setTextCatalogName(f.name.replace(/\.(txt|csv)$/i, ""));
                      }
                      e.target.value = "";
                    }}
                  />

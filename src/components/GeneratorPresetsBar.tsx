...
export default function GeneratorPresetsBar({
  ...
  const [textImportTarget, setTextImportTarget] = useState<ImportTarget>("new");
  const [textImportPresetId, setTextImportPresetId] = useState("");
  const [textMergeMode, setTextMergeMode] = useState<"append" | "replace">("append");
  ...
  const readTextFileSmart = async (file: File): Promise<string> => {
    const buf = await file.arrayBuffer();
    try {
      const utf8 = new TextDecoder("utf-8", { fatal: true }).decode(buf);
      return stripBom(utf8);
    } catch {
      /* try legacy Chinese encodings */
    }
    for (const enc of ["gb18030", "gbk", "gb2312"] as const) {
      try {
        return stripBom(new TextDecoder(enc).decode(buf));
      } catch {
  ...
  const openTextImport = () => {
    setTextImportPresetId(selectValue || presets[0]?.id || "");
    setTextImportTarget(presets.length ? "existing" : "new");
  ...
  const runTextImport = () => {
    try {
      ...
      if (textImportTarget === "existing") {
        const pid = textImportPresetId || selectValue;
        if (!pid) {
          toast("请选择目标预设");
          return;
        }
        const updated = mergeSectionIntoPreset(pid, section, {
          replaceItems: textMergeMode === "replace",
        });
        refresh(updated.data, updated.id);
        setTextImportOpen(false);
        setTextRaw("");
        toast(
          `已${textMergeMode === "replace" ? "覆盖" : "合并"} ${section.items.length} 条到「${updated.name}」`
        );
      } else {
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-neutral-800 pt-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" onClick={openCsvImport} className="px-3 py-1.5 text-xs rounded-lg border border-sky-800 text-sky-300 hover:bg-sky-950/40">从 CSV / TSV 导入</button>
            <button type="button" onClick={openTextImport} className="px-3 py-1.5 text-xs rounded-lg border border-amber-800/70 text-amber-300 hover:bg-amber-950/30">从文本列表导入（推荐中文）</button>
          </div>
        </div>
      )}
      ...
      {csvImportOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-[#111] border border-neutral-700 rounded-xl p-5 space-y-4 max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-semibold text-white">从 CSV / TSV 导入词库</h2>
            <p className="text-xs text-neutral-500 leading-relaxed">
              每行两列：<code className="text-neutral-300">名字,提示词</code>
              （也可用 Tab 分隔）。第一行可以是表头。提示词含逗号时请用双引号包裹。
              <br />
              <span className="text-amber-400/90">Excel 请另存为「CSV UTF-8」</span>
              ，或直接从表格复制粘贴到下方（自动识别 Tab）。简体中文推荐改用「文本列表导入」。
            </p>

            <div className="rounded-lg border border-neutral-800 bg-[#0c0c0c] p-3 space-y-2">
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs text-neutral-400">参考范例</span>
                <button type="button" onClick={downloadSampleCsv} className="text-[11px] text-sky-400 hover:text-sky-300">下载范例 CSV</button>
              </div>
              <pre className="text-[11px] font-mono text-neutral-400 whitespace-pre-wrap break-all leading-relaxed">{CSV_IMPORT_SAMPLE}</pre>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs text-neutral-500">CSV / TSV 内容</label>
                <div>
                  <input
                    ref={csvFileRef}
                    type="file"
                    ...
                    onChange={async (e) => {
                      ...
                      if (!f) return;
                      setCsvRaw(await readTextFileSmart(f));
                      if (!csvCatalogName.trim()) {
                        setCsvCatalogName(f.name.replace(/\.(csv|tsv|txt)$/i, ""));
                      }
                      e.target.value = "";
                    }}
                  />
                  <button type="button" onClick={() => csvFileRef.current?.click()} className="text-[11px] text-sky-400 hover:text-sky-300">从文件载入</button>
                </div>
              </div>
              <textarea
      ...
      {textImportOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-[#111] border border-neutral-700 rounded-xl p-5 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-xs text-neutral-500 leading-relaxed">
              每行一条：<code className="text-neutral-300">名称: 描述</code>
              。适合中文词条，无需 CSV 编码。
            </p>
            <div>
              <label className="text-xs text-neutral-500 block mb-1">导入目标</label>
              <div className="flex gap-2">
                <button type="button" onClick={() => setTextImportTarget("new")} className={`flex-1 px-3 py-1.5 text-sm rounded-lg border ${textImportTarget === "new" ? "border-purple-500 text-purple-200" : "border-neutral-700 text-neutral-400"}`}>新建预设</button>
                <button type="button" onClick={() => setTextImportTarget("existing")} className={`flex-1 px-3 py-1.5 text-sm rounded-lg border ${textImportTarget === "existing" ? "border-purple-500 text-purple-200" : "border-neutral-700 text-neutral-400"}`}>合并到已有预设</button>
              </div>
            </div>
            {textImportTarget === "existing" && (
              <div className="space-y-2">
                <div>
                  <label className="text-xs text-neutral-500 block mb-1">目标预设</label>
                  <select className="w-full bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2 text-sm outline-none focus:border-purple-500" value={presets.some((p) => p.id === textImportPresetId) ? textImportPresetId : presets[0]?.id || ""} onChange={(e) => setTextImportPresetId(e.target.value)}>
                    {presets.length === 0 && <option value="">（无预设）</option>}
                    {presets.map((p) => (
                      <option key={p.id} value={p.id}>{p.name}</option>
                    ))}
                ...
                <div>
                  <label className="text-xs text-neutral-500 block mb-1">若分区 key 已存在</label>
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setTextMergeMode("append")} className={`flex-1 px-3 py-1.5 text-xs rounded-lg border ${textMergeMode === "append" ? "border-emerald-600 text-emerald-200" : "border-neutral-700 text-neutral-400"}`}>追加词条</button>
                    <button type="button" onClick={() => setTextMergeMode("replace")} className={`flex-1 px-3 py-1.5 text-xs rounded-lg border ${textMergeMode === "replace" ? "border-amber-600 text-amber-200" : "border-neutral-700 text-neutral-400"}`}>覆盖整个分区</button>
                  </div>
                </div>
              </div>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {textImportTarget === "new" && (
                <div className="sm:col-span-2">
                  <label className="text-xs text-neutral-500 block mb-1">新预设名称</label>
                  <input className="w-full bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2 text-sm outline-none focus:border-purple-500" value={textCatalogName} onChange={(e) => setTextCatalogName(e.target.value)} placeholder="例如：发型词库" />
                </div>
              )}
              <div>
                <label className="text-xs text-neutral-500 block mb-1">分区名称</label>
                ...
                <div>
                  <input
                    ...
                    onChange={async (e) => {
                      const f = e.target.files?.[0];
                      if (!f) return;
                      setTextRaw(await readTextFileSmart(f));
                      if (!textCatalogName.trim()) {
                        setTextCatalogName(f.name.replace(/\.txt$/i, ""));
                      }
                      e.target.value = "";
                    }}
                  />
                  <button type="button" onClick={() => textFileRef.current?.click()} className="text-[11px] text-sky-400 hover:text-sky-300">从 .txt 载入</button>
                </div>
              </div>
              <textarea
                className="w-full min-h-[160px] bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2 text-xs outline-none focus:border-purple-500 resize-y"
                value={textRaw}
                onChange={(e) => setTextRaw(e.target.value)}
                placeholder={"金发: blonde hair, long hair,\n红瞳: red eyes, "} 
              />
            </div>
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setTextImportOpen(false)} className="px-4 py-2 text-sm text-neutral-400">取消</button>
              <button type="button" onClick={runTextImport} disabled={!textRaw.trim()} className="px-4 py-2 text-sm rounded-lg bg-purple-600 hover:bg-purple-500 disabled:opacity-40 text-white">导入</button>
            </div>
          </div>
        </div>
      )}

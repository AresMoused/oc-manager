    ...
    <div className="bg-[#111] border border-neutral-800 rounded-xl p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <button
          ...
          className="px-3 py-1.5 text-xs rounded-lg border border-rose-900/50 text-rose-400 hover:bg-rose-950/30"
        >
          删除当前
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {onOverwriteCurrent && (
          <button
            ...
            className="px-3 py-1.5 text-xs rounded-lg border border-emerald-800 text-emerald-300 hover:bg-emerald-950/40"
            title="把当前编辑结果写回当前选中的预设"
          >
            保存到当前预设
          </button>
        )}
        ...
        {BUILTIN_CATALOGS.map((c) => (
          <button
            key={c.id}
            type="button"
            onClick={() => handleLoadBuiltin(c.file, c.name)}
            className="px-2.5 py-1 text-[11px] rounded-lg border border-neutral-700 text-neutral-300 hover:bg-neutral-800"
          >
            加载内置：{c.name}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        <input
          className="bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2 text-xs outline-none focus:border-purple-500"
        ...
        <div className="flex gap-2">
          <input
            className="flex-1 bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2 text-xs font-mono outline-none focus:border-purple-500"
            placeholder="从 URL 加载 JSON…"
            value={newPresetUrl}
          ...
          <button
            ...
            className="px-3 py-1.5 text-xs rounded-lg bg-purple-700 hover:bg-purple-600 text-white shrink-0"
          >
            添加
          </button>
        </div>
      ...
      <div>
        ...
        <button
          ...
          className="px-3 py-1.5 text-xs rounded-lg border border-neutral-700 text-neutral-300 hover:bg-neutral-800"
        >
          从 JSON 文件导入为新预设
        </button>
      </div>

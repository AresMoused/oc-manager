    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 py-6 space-y-5">
        <div>
          ...
          <p className="text-neutral-500 text-sm mt-1">在下方输入人设需求后生成，结果可编辑再导入为角色卡。API / 预设在底部「设定」。</p>
          <div className="mt-2 flex flex-wrap gap-2 text-xs text-neutral-500">
            ...
            {!cfg.apiKey && (
              <button type="button" onClick={() => setSettingsOpen(true)} className="px-2 py-0.5 rounded bg-amber-950/40 border border-amber-800/50 text-amber-300">未配置 API Key · 去设定</button>
            )}
          </div>
        </div>

        <section className="bg-[#141414] border border-neutral-800 rounded-2xl p-6 sm:p-8 space-y-5">
          <h2 className="text-center text-lg font-semibold text-white tracking-wide">角色人设生成</h2>
          <div className="border-t border-neutral-800" />
          <p className="text-center text-sm text-neutral-400">请输入角色人设生成需求（可选），留空则仅依据上下文预设生成</p>
          {active && active.entries.some((e) => e.enabled) && (
            <div className="text-xs text-neutral-500 bg-neutral-900/60 border border-neutral-800 rounded-lg px-3 py-2 line-clamp-3">
              <span className="text-neutral-400">当前预设：</span>{active.name}（{active.entries.filter((e) => e.enabled).length} 条启用）
            </div>
          )}
          <textarea
            className="w-full min-h-[140px] bg-[#1a1a1a] border-2 border-purple-600/70 focus:border-purple-500 rounded-2xl px-4 py-3.5 text-sm outline-none text-neutral-200 resize-y placeholder:text-neutral-600"
            placeholder="例如：生成一个温柔的猫娘角色人设，性格害羞但内心善良…"
            value={userPrompt} onChange={(e) => setUserPrompt(e.target.value)}
          />
          <div className="rounded-2xl border border-dashed border-neutral-700 bg-[#121212] p-4">
            <div className="flex items-center justify-between gap-2 mb-2">
              <span className="text-sm text-neutral-400">参考图片（可选）</span>
              <button type="button" onClick={() => imageFileRef.current?.click()} className="px-3 py-1.5 text-sm rounded-lg border border-purple-600/60 text-purple-300 hover:bg-purple-950/30">+ 添加图片</button>
              <input ref={imageFileRef} type="file" accept="image/*" multiple className="hidden" onChange={(e) => {
                Array.from(e.target.files || []).forEach((file) => {
                  const reader = new FileReader();
                  reader.onload = () => setRefImages((prev) => [...prev, { id: newId(), name: file.name, dataUrl: String(reader.result || "") }]);
            ...
            {refImages.length === 0 ? (
              <p className="text-center text-xs text-neutral-600 py-4">点击上方按钮添加参考图片</p>
            ) : (
              <div className="flex flex-wrap gap-3">
                {refImages.map((img) => (
                  <div key={img.id} className="relative">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={img.dataUrl} alt={img.name} className="w-20 h-20 object-cover rounded-lg border border-neutral-700" />
                    <button type="button" onClick={() => setRefImages((prev) => prev.filter((x) => x.id !== img.id))} className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-rose-600 text-white text-xs">×</button>
                  </div>
                ))}
              </div>
            )}
          </div>
          {error && <p className="text-sm text-rose-400 text-center break-all">{error}</p>}
          <div className="flex items-center justify-center gap-3 pt-1">
            <button type="button" onClick={() => { setUserPrompt(""); setOutput(""); setError(""); setRefImages([]); }} className="px-6 py-2.5 rounded-xl border border-neutral-700 text-neutral-300 hover:bg-neutral-800 text-sm">取消</button>
            {!generating ? (
              <button type="button" onClick={handleGenerate} className="px-8 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-sm font-medium shadow-lg shadow-purple-900/30">开始生成</button>
            ) : (
              <button type="button" onClick={() => abortRef.current?.abort()} className="px-8 py-2.5 rounded-xl bg-rose-700 hover:bg-rose-600 text-white text-sm font-medium">停止生成</button>
            )}
          </div>
        </section>
        ...
        {(output || generating) && (
          <section className="bg-[#141414] border border-neutral-800 rounded-2xl p-5 sm:p-6 space-y-4">
            ...
            <textarea
              ...
              value={output} onChange={(e) => setOutput(e.target.value)} placeholder="生成内容将显示在这里…" readOnly={generating}
            />
            ...
            <div className="border-t border-neutral-800 px-5 py-5 space-y-8">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input className="w-full bg-[#1a1a1a] border border-neutral-700 rounded-lg px-3 py-2.5 text-sm font-mono outline-none focus:border-purple-500 text-neutral-200" value={cfg.baseUrl} onChange={(e) => persistCfg({ ...cfg, baseUrl: e.target.value })} placeholder="https://api.deepseek.com" />
                </div>
                <div>
                  <label className="text-xs text-neutral-500 mb-1.5 block">API Key（仅本浏览器）</label>
                  <div className="relative">
                    <input type={showKey ? "text" : "password"} className="w-full bg-[#1a1a1a] border border-neutral-700 rounded-lg px-3 py-2.5 text-sm font-mono outline-none focus:border-purple-500 text-neutral-200 pr-10" value={cfg.apiKey} onChange={(e) => persistCfg({ ...cfg, apiKey: e.target.value })} placeholder="sk-..." autoComplete="off" />
                    <button type="button" onClick={() => setShowKey((v) => !v)} className="absolute right-2 top-1/2 -translate-y-1/2 text-neutral-500 hover:text-white text-xs px-2">{showKey ? "隐藏" : "显示"}</button>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    {cfg.models.length > 0 ? cfg.models.slice(0, 12).map((m) => (
                      <button key={m} type="button" onClick={() => persistCfg({ ...cfg, model: m })} className={`px-3 py-1.5 text-xs rounded-lg border transition ${cfg.model === m ? "border-purple-500 bg-purple-950/40 text-purple-200" : "border-neutral-700 text-neutral-400"}`}>{m}</button>
                    )) : (
                      <input className="flex-1 min-w-[160px] bg-[#1a1a1a] border border-neutral-700 rounded-lg px-3 py-2 text-sm font-mono outline-none focus:border-purple-500" value={cfg.model} onChange={(e) => persistCfg({ ...cfg, model: e.target.value })} placeholder="deepseek-chat" />
                    )}
                    <button type="button" onClick={handleFetchModels} disabled={fetchingModels} className="px-3 py-1.5 text-sm rounded-lg border border-neutral-700 text-neutral-300 hover:bg-neutral-800 disabled:opacity-50">{fetchingModels ? "获取中…" : "连接并获取模型"}</button>
                  </div>
                  {cfg.models.length > 0 && (
                    <input className="mt-2 w-full bg-[#1a1a1a] border border-neutral-700 rounded-lg px-3 py-2 text-xs font-mono outline-none focus:border-purple-500" value={cfg.model} onChange={(e) => persistCfg({ ...cfg, model: e.target.value })} list="ai-models" />
                  )}
                ...
                {toggles.map(([key, label, val]) => (
                  <div key={key} className="flex items-center justify-between gap-4">
                    ...
                    <button type="button" role="switch" aria-checked={val} onClick={() => persistCfg({ ...cfg, [key]: !val })} className={`w-11 h-6 rounded-full relative ${val ? "bg-purple-600" : "bg-neutral-700"}`}>
              ...
              <div className="space-y-4">
                ...
                {paramRows.map(([key, label, min, max, step, val]) => (
                  <div key={key} className="flex flex-wrap items-center gap-3">
                    ...
                    <input type="range" min={min} max={max} step={step} value={val} onChange={(e) => persistParams({ ...params, [key]: Number(e.target.value) })} className="flex-1 min-w-[120px] accent-purple-500" />
                    <input type="number" min={min} max={max} step={step} value={val} onChange={(e) => persistParams({ ...params, [key]: Number(e.target.value) })} className="w-20 bg-[#1a1a1a] border border-neutral-700 rounded-lg px-2 py-1.5 text-sm text-purple-300 tabular-nums outline-none focus:border-purple-500" />
                  </div>
                ))}
              </div>
              <div className="space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <select className="bg-[#1a1a1a] border border-neutral-700 rounded-lg px-3 py-1.5 text-sm outline-none focus:border-purple-500 max-w-[160px]" value={activeId} onChange={(e) => { setActiveId(e.target.value); saveActivePresetId(e.target.value); }}>
                    ...
                    <button type="button" onClick={() => { const name = prompt("预设名称", active?.name || ""); if (name != null && active) updateActive({ name: name.trim() || active.name }); }} className="px-2.5 h-8 rounded-lg border border-neutral-700 text-neutral-300 text-xs">重命名</button>
                    <button type="button" onClick={() => presetFileRef.current?.click()} className="px-2.5 h-8 rounded-lg border border-purple-800/60 text-purple-300 text-xs">导入预设</button>
                ...
                <p className="text-xs text-neutral-500">支持导入单个预设、预设数组或 {"{ presets: [...] }"} JSON。</p>
                  ...
                  {(active?.entries || []).map((entry) => (
                    <div key={entry.id} className={`flex items-start gap-2 p-3 rounded-lg border bg-[#0c0c0c] ${entry.enabled ? "border-neutral-700" : "border-neutral-800 opacity-50"}`}>
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <button type="button" role="switch" aria-checked={entry.enabled} onClick={() => updateEntry(entry.id, { enabled: !entry.enabled })} className={`w-9 h-5 rounded-full relative ${entry.enabled ? "bg-purple-600" : "bg-neutral-700"}`}>
      ...
      {editingEntry && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-[#111] border border-neutral-700 rounded-xl p-5 space-y-3">
            ...
            <input className="w-full bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2 text-sm outline-none focus:border-purple-500" value={editingEntry.name} onChange={(e) => setEditingEntry({ ...editingEntry, name: e.target.value })} />

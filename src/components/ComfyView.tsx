...
export default function ComfyView() {
  ...
  useEffect(() => {
    ...
    async function bootBuilder() {
      let source = getActivePreset()?.data || loadCachedBuilder();
      if (!source) {
        try {
    ...
    const reloadBuilder = () => {
      const source = getActivePreset()?.data || loadCachedBuilder();
      if (source) setBuilder(source);
    };
  ...
  const rollRandomCharacter = useCallback((data: BuilderData | null): string | null => {
    ...
    // Respect appearance-generator section on/off toggles (oc-gen-sec-on-*)
  ...
  const sectionCount = builder
    ? filterEnabledSections(builder).sections.length
    : 0;

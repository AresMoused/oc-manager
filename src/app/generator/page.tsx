...
export default function GeneratorPage() {
  ...
  const toastMsg = (m: string) => { setToast(m); setTimeout(() => setToast(""), 2000); };

  const reload = useCallback(async (ids: string[], fx: string) => {
    const data = await buildEnabledBuilderData(ids, fx);
    setSections(data.sections);
    try { localStorage.setItem("oc-lexicon-runtime-builder", JSON.stringify(data)); } catch {}
  }, []);
            ...
            {pending.map(p => (
              <div key={p.id} className="flex gap-2 text-xs items-center">
                ...
                <button className="text-rose-400 border border-rose-800 px-2 rounded" onClick={async () => {
                  ...
                  setPending(x => x.filter(i => i.id !== p.id)); toastMsg("已拒绝");
                }}>拒绝</button>
              </div>
            ))}
          </div>
          ...
          <div className="w-full max-w-md bg-[#111] border border-neutral-700 rounded-xl p-4 space-y-2">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button className="bg-purple-600 text-white text-sm px-3 py-1 rounded" onClick={async () => {
                const items = upRaw.split(/\r?\n/).filter(Boolean).map(line => {
                  const m = line.match(/^(.+?)[:：]\s*(.*)$/);
                  return m ? { name: m[1]!.trim(), tags: m[2]!.trim() || m[1]! } : { name: line.trim(), tags: line.trim() + ", " };
                });
